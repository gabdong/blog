...
apis.get('/refresh', (req, res) => {
    ...
    const user = accessToken ? token().check(accessToken, 'access') || token().check(refreshToken, 'refresh') : token().check(refreshToken, 'refresh');
...
apis.get('/verify-token', (req, res) => {
    ...
    const result = {};
    const verify = accessToken ? token().check(accessToken, 'access') : token().check(refreshToken, 'refresh');
